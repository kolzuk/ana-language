...
bool Scope::insert(Decl* Declaration) {
  return ScopeVariables.insert(std::pair<std::string, Decl*>(Declaration->Name, Declaration)).second;
}

bool Scope::insertFunc(Decl* Declaration) {
  return ScopeFunc.insert(std::pair<std::string, Decl*>(Declaration->Name, Declaration)).second;
}

Decl* Scope::lookup(const std::string& Name) {
  Scope* CurrentScp = this;
  while (CurrentScp) {
    auto Iter = CurrentScp->ScopeVariables.find(Name);
    if (Iter != CurrentScp->ScopeVariables.end())
      return Iter->second;
...
Decl* Scope::lookupFunc(const std::string& Name) {
  Scope* CurrentScp = this;
  while (CurrentScp) {
    auto Iter = CurrentScp->ScopeFunc.find(Name);
    if (Iter != CurrentScp->ScopeFunc.end())
      return Iter->second;
    CurrentScp = CurrentScp->getParent();
  }
  return nullptr;
...
bool SemaVisitor::hasError() const { return HasError; }
...
void SemaVisitor::visit(FunctionDeclarationAST& Node) {
  ...
  if (HasError) return;
  Scp->insertFunc(new Decl(Name, Node.ReturnType, true, nullptr));
  Decl* Fun = Scp->lookupFunc(Name);
...
void SemaVisitor::visit(WhileStatementAST& Node) {
  ...
  Scope* ParentScope = Scp;
  Scp = new Scope("while", ParentScope);
  if (HasError) return;
  Node.Body->accept(*this);
  delete Scp;
  Scp = ParentScope;
...
void SemaVisitor::visit(ForStatementAST& Node) {
  if (HasError) return;
  Scope* ParentScope = Scp;
  Scp = new Scope("for", ParentScope);
...
void SemaVisitor::visit(ExpressionsListAST& Node) {
  Decl* Func = Scp->lookupFunc(LastFunctionName);
  if (!Func->Arguments) {
    HasError = true;
    return;
  }
